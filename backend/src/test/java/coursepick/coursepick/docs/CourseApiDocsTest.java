...
public class CourseApiDocsTest extends AbstractApiDocsSupport {
    ...
    void 좌표_근처_코스_전체_조회_API() throws Exception {
        ...
        given(courseApplicationService.findNearbyCourses(any(CourseFindCondition.class), anyDouble(), anyDouble()))
        ...
        mockMvc.perform(get("/v1/courses")
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("course-find-nearby",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .queryParameters(
                                        parameterWithName("mapLat").description("지도 중심의 위도(-90 ~ 90)").attributes(key("example").value("37.5165")),
                                        parameterWithName("mapLng").description("지도 중심의 경도(-180 ~ 180)").attributes(key("example").value("127.1040")),
                                        parameterWithName("scope").description("좌표 중심으로부터 탐색하고자 하는 범위 (미터)").attributes(key("example").value("1000")),
                                        parameterWithName("userLat").description(사용자_위도).optional().attributes(key("example").value("37.5165")),
                                        parameterWithName("userLng").description(사용자_경도).optional().attributes(key("example").value("127.1040")),
                                        parameterWithName("minLength").description("최소 코스 길이").optional().attributes(key("example").value("0")),
                                        parameterWithName("maxLength").description("최대 코스 길이").optional().attributes(key("example").value("10000")),
                                        parameterWithName("page").description("페이지 번호").optional().attributes(key("example").value("0"))
                                )
                                .responseFields(
                                        fieldWithPath("courses[].id").description(코스_ID),
                                        fieldWithPath("courses[].name").description(코스_이름),
                                        fieldWithPath("courses[].distance").description(사용자_위치로부터의_거리).optional(),
                                        fieldWithPath("courses[].length").description(코스_전체_길이),
                                        fieldWithPath("courses[].coordinates[].latitude").description(위도),
                                        fieldWithPath("courses[].coordinates[].longitude").description(경도),
                                        fieldWithPath("courses[].creatorId").description(코스_등록_주체_ID),
                                        fieldWithPath("hasNext").description(다음_페이지_존재_여부)
                                )
                                .build()
                        )
                ));
        ...
        mockMvc.perform(get("/v1/courses/{id}/closest-coordinate", "69d8c0b55561463adc32f259")
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("course-closest-coordinate",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .pathParameters(
                                        parameterWithName("id").description(코스_ID).attributes(key("example").value("69d8c0b55561463adc32f259"))
                                )
                                .queryParameters(
                                        parameterWithName("lat").description(사용자_위도).attributes(key("example").value("37.5165")),
                                        parameterWithName("lng").description(사용자_경도).attributes(key("example").value("127.1040"))
                                )
                                .responseFields(
        ...
        mockMvc.perform(get("/v1/courses/{id}/route", "69d8c0b55561463adc32f259")
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("course-route",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .queryParameters(
                                        parameterWithName("startLat").description(사용자_위도).attributes(key("example").value("37.5165")),
                                        parameterWithName("startLng").description(사용자_경도).attributes(key("example").value("127.1040"))
                                )
                                .responseFields(
                                        fieldWithPath("[].latitude").description(위도),
                                        fieldWithPath("[].longitude").description(경도)
    ...
    void 코스_상세_조회_API() throws Exception {
        CourseDetailResponse detailResponse = new CourseDetailResponse(
                ...
                1,
                5.0,
                List.of(new ReviewResponse("69d8c0b54561463adc32f259", "착한 강아지", "69d8c0b12361463adc32f259", "노을이 예뻐요.", 5))
        );
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .pathParameters(
                                        parameterWithName("id")
                                                .description(코스_ID)
                                                .attributes(key("example").value("69d8c0b55561463adc32f259"))
                                )
                                .responseFields(
                                        fieldWithPath("id").description(코스_ID),
                                        fieldWithPath("name").description(코스_이름),
                                        fieldWithPath("length").description(코스_전체_길이),
                                        fieldWithPath("coordinates[].latitude").description(위도),
                                        fieldWithPath("coordinates[].longitude").description(경도),
                                        fieldWithPath("reviewOverview.reviewCount").description("리뷰 개수"),
                                        fieldWithPath("reviewOverview.averageRating").description("리뷰 평균 별점"),
                                        fieldWithPath("reviews[].id").description("리뷰 ID"),
                                        fieldWithPath("reviews[].rating").description(리뷰_별점),
                                        fieldWithPath("reviews[].authorId").description("리뷰 작성자 ID"),
                                        fieldWithPath("reviews[].authorNickname").description("리뷰 작성자 닉네임"),
                                        fieldWithPath("reviews[].content").description(리뷰_내용)
                                )
                                .build()
                        )
                ));
        ...
        mockMvc.perform(get("/v1/courses/favorites")
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("course-favorites",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .summary("즐겨찾기 코스 조회")
                                .queryParameters(
                                        parameterWithName("courseIds").description("코스 ID 목록 (쉼표 구분)").attributes(key("example").value("689c3143182cecc6353cca7b,689c3143182cecc6353cca7c"))
                                )
                                .responseFields(
                                        fieldWithPath("[].id").description(코스_ID),
                                        fieldWithPath("[].name").description(코스_이름),
                                        fieldWithPath("[].distance").description(사용자_위치로부터의_거리).optional(),
                                        fieldWithPath("[].length").description(코스_전체_길이),
                                        fieldWithPath("[].coordinates[].latitude").description(위도),
                                        fieldWithPath("[].coordinates[].longitude").description(경도),
        ...
        mockMvc.perform(post("/v1/courses/draft/route")
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("course-draft-route",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .requestFields(
                                        fieldWithPath("origin.latitude").description("직전 포인트 위도").attributes(key("example").value("37.514167")),
                                        fieldWithPath("origin.longitude").description("직전 포인트 경도").attributes(key("example").value("127.103611")),
                                        fieldWithPath("destination.latitude").description("새 포인트 위도").attributes(key("example").value("37.515167")),
                                        fieldWithPath("destination.longitude").description("새 포인트 경도").attributes(key("example").value("127.104611"))
                                )
                                .responseFields(
                                        fieldWithPath("coordinates[].latitude").description(위도),
                                        fieldWithPath("coordinates[].longitude").description(경도),
        ...
        mockMvc.perform(post("/v1/courses/{id}/reviews", "689c3143182cecc6353cca7b")
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("course-add-review",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .requestFields(
                                        fieldWithPath("content").description(리뷰_내용).attributes(key("example").value("정말 멋진 러닝 코스입니다!")),
                                        fieldWithPath("rating").description(리뷰_별점).attributes(key("example").value(5))
                                )
                                .build()
                        )
                ));
        ...
        mockMvc.perform(post("/v1/courses")
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("course-add-custom",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .requestFields(
                                        fieldWithPath("name").description("코스 이름").attributes(key("example").value("나만의 한강 러닝 코스")),
                                        fieldWithPath("coordinates[].latitude").description(위도).attributes(key("example").value("37.514167")),
                                        fieldWithPath("coordinates[].longitude").description(경도).attributes(key("example").value("127.103611"))
                                )
                                .build()
                        )
                ));
        ...
        mockMvc.perform(get("/v1/courses/custom")
                ...
                .andDo(MockMvcRestDocumentationWrapper.document("course-find-custom",
                        ...
                        resource(ResourceSnippetParameters.builder()
                                ...
                                .queryParameters(
                                        parameterWithName("userLat").description(사용자_위도).optional().attributes(key("example").value("37.516")),
                                        parameterWithName("userLng").description(사용자_경도).optional().attributes(key("example").value("127.104"))
                                )
                                .responseFields(
                                        fieldWithPath("courses[].id").description(코스_ID),
                                        fieldWithPath("courses[].name").description(코스_이름),
