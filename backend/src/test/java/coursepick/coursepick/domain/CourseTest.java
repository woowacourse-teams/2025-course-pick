...
class CourseTest {
    ...
    class 생성_테스트 {
        ...
        @Test
        void 코스를_생성한다() {
            assertThatCode(() -> new Course("코스이름", "행정구역정보", getNormalCoordinates()))
                    .doesNotThrowAnyException();
        }

        @Test
        void 앞_뒤_공백을_제거하여_생성한다() {
            var course = new Course(" 코스이름   ", "행정구역정보", getNormalCoordinates());
        ...
        void 잘못된_길이의_이름으로_코스를_생성하면_예외가_발생한다(String name) {
            assertThatThrownBy(() -> new Course(name, "행정구역정보", getNormalCoordinates()))
        ...
        void 이름의_연속공백을_한_칸으로_변환하여_코스를_생성한다(String name) {
            var course = new Course(name, "행정구역정보", getNormalCoordinates());
            assertThat(course.name()).isEqualTo("코스 이름");
        }

        @Test
        void 코스의_좌표의_개수가_2보다_적으면_예외가_발생한다() {
            assertThatThrownBy(() -> new Course("코스이름", "행정구역정보", List.of(new Coordinate(1d, 1d))))
                    .isInstanceOf(IllegalArgumentException.class);
        }

        @Test
        void 반시계방향이면_그대로_설정된다() {
            var counter = new Course("코스이름", "행정구역정보", List.of(
        ...
        void 시계방향이면_반대로_설정된다() {
            var clockwiseCourse = new Course("코스이름", "행정구역정보", List.of(
                    new Coordinate(0d, 0d),
                    new Coordinate(5d, -5d),
                    new Coordinate(0d, 10d),
                    new Coordinate(5d, 5d),
    ...
    void 코스의_총_거리를_계산할_수_있다() {
        var course = new Course("한강뛰어보자", "서울 송파구 잠실본동", List.of(
    ...
    void 코스_위의_점에서_코스까지의_거리는_0에_가깝다() {
        var course = new Course("직선코스", "행정구역정보", List.of(
    ...
    void 코스_시작점에서_코스까지의_거리는_0이다() {
        var course = new Course("삼각형코스", "행정구역정보", List.of(
                new Coordinate(37.5, 127.0),
    ...
    void 코스_내부의_점에서_코스까지의_거리를_계산한다() {
        var course = new Course("사각형코스", "행정구역정보", List.of(
                new Coordinate(37.5, 127.0),
                new Coordinate(37.501, 127.0),
    ...
    void 코스_외부_멀리_떨어진_점에서_코스까지의_거리를_계산한다() {
        var course = new Course("작은원형코스", "행정구역정보", List.of(
                new Coordinate(37.5, 127.0),
    ...
    void 코스_주변_다양한_위치에서의_거리를_계산한다(double latitude, double longitude, int expectedDistance) {
        var course = new Course("정사각형코스", "행정구역정보", List.of(
                new Coordinate(37.5, 127.0),
                new Coordinate(37.5001, 127.0),
    ...
    void 코스의_좌표_중에서_가장_가까운_좌표를_계산한다(double targetLatitude, double targetLongitude, double latitude, double longitude) {
        Course course = new Course("왕복코스", "행정구역정보", List.of(
                new Coordinate(0, 0),
    ...
    void 코스의_난이도를_계산한다(List<Coordinate> coordinates, RoadType roadType, double expectedDifficulty) {
        var course = new Course("코스", "행정구역정보", roadType, coordinates);
