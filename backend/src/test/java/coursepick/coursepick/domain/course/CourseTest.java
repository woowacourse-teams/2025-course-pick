...
class CourseTest {
    ...
    void 코스의_총_길이를_계산할_수_있다() {
        var course = new Course(null, "코스", square(new Coordinate(0, 0), 10000, 10000), null);
    ...
    void 특정_좌표에서_코스까지_가장_가까운_거리를_계산할_수_있다(Coordinate target, double expectedDistance) {
        var base = new Coordinate(0, 0);
        var course = new Course(null, "코스", square(base, 10000, 10000), null);
    ...
    class 생성_테스트 {
        ...
        @Test
        void 코스를_생성한다() {
            assertThatCode(() -> new Course(null, "코스", List.of(new Coordinate(0, 0), new Coordinate(2, 2)), null))
                    .doesNotThrowAnyException();
        }

        @Test
        void 유저가_만든_코스를_생성한다() {
            assertThatCode(() -> new Course(null, "코스", List.of(new Coordinate(0, 0), new Coordinate(2, 2)), CourseCreator.of("userId")))
                    .doesNotThrowAnyException();
        }

        @Test
        void 앞_뒤_공백을_제거하여_생성한다() {
            var course = new Course(null, " 코스   ", List.of(new Coordinate(0, 0), new Coordinate(2, 2)), CourseCreator.of("userId"));
        ...
        void 잘못된_길이의_이름으로_코스를_생성하면_예외가_발생한다(String name) {
            assertThatThrownBy(() -> new Course(null, name, List.of(new Coordinate(0, 0), new Coordinate(2, 2)), null))
        ...
        void 이름의_연속공백을_한_칸으로_변환하여_코스를_생성한다(String name) {
            var course = new Course(null, name, List.of(new Coordinate(0, 0), new Coordinate(2, 2)), null);
            assertThat(course.name().value()).isEqualTo("코스 이름");
        }

        @Test
        void 코스의_좌표의_개수가_2보다_적으면_예외가_발생한다() {
            assertThatThrownBy(() -> new Course(null, "코스", List.of(new Coordinate(0, 0)), null))
                    .isInstanceOf(IllegalArgumentException.class);
        }

        @Test
        void 코스_생성시_첫_좌표_끝_좌표만_존재할때_둘은_중복될_수_없다() {
            assertThatThrownBy(() -> new Course(null, "코스", List.of(new Coordinate(0, 0), new Coordinate(0, 0)), null))
                    .isInstanceOf(IllegalArgumentException.class);
        }
    }
}
